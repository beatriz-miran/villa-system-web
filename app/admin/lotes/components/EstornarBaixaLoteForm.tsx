...
import {
  estornarBaixaLoteAction,
  type EstornarBaixaLoteActionState,
} from "@/app/admin/lotes/actions";

type EstornarBaixaLoteFormProps = {
  baixaId: number;
  tipo: string;
  quantidade: number;
  data: string;
...
const estadoInicial: EstornarBaixaLoteActionState =
  {};
  ...
  [
    {
      ...
      texto:
        "Estorno realizado porque a quantidade de aves foi informada incorretamente.",
    },
    {
      id: "data",
      titulo: "Data incorreta",
      texto:
        "Estorno realizado porque a data da ocorrência foi informada incorretamente.",
    },
    {
      id: "tipo",
      titulo: "Tipo incorreto",
      texto:
        "Estorno realizado porque o tipo de baixa foi selecionado incorretamente.",
    },
    {
      id: "duplicidade",
      titulo: "Registro duplicado",
      texto:
        "Estorno realizado porque esta baixa foi registrada em duplicidade.",
    },
    {
      id: "lote",
      titulo: "Lote incorreto",
      texto:
        "Estorno realizado porque a baixa foi vinculada ao lote incorreto.",
    },
  ];

export default function EstornarBaixaLoteForm({
  baixaId,
  tipo,
  quantidade,
  data,
}: EstornarBaixaLoteFormProps) {
  ...
  const [state, formAction, pendente] =
    useActionState(
      async (
        estadoAnterior: EstornarBaixaLoteActionState,
        formData: FormData,
      ) => {
        const resultado =
          await estornarBaixaLoteAction(
            estadoAnterior,
            formData,
          );
      ...
      {modalAberto ? (
        ...
        >
          <div
            role="dialog"
            aria-modal="true"
            aria-labelledby={`titulo-estorno-baixa-${baixaId}`}
          ...
          >
            ...
            >
              ...
              <header className="flex items-start justify-between gap-4 border-b border-gray-200 px-5 py-4">
                <div>
                  <h2
                    id={`titulo-estorno-baixa-${baixaId}`}
              ...
              <div className="space-y-5 px-5 py-5">
                ...
                <div className="rounded-lg border border-amber-200 bg-amber-50 px-4 py-3">
                  ...
                  <p className="mt-1 text-sm leading-5 text-amber-700">
                    A quantidade deste registro retornará ao saldo do lote. O lançamento permanecerá visível no histórico como estornado.
                  </p>
                </div>

                <div>
                  <label
                    htmlFor={`modelo-estorno-${baixaId}`}
                ...
                <div>
                  <div className="flex items-center justify-between gap-3">
                    <label
                      htmlFor={`motivo-estorno-${baixaId}`}
                      className="text-sm font-semibold text-gray-800"
                    >
